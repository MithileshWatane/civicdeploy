...
export default function CommunityUpdated() {
  ...
  const fetchProjects = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:10000/api/community/projects', {
        headers: {
      ...
      const projectsWithDetails = await Promise.all(
        response.data.map(async (project) => {
          ...
          if (project.createdBy) {
            try {
              const userResponse = await axios.get(`http://localhost:10000/api/users/${project.createdBy}`);
  ...
  const handleContribution = async (projectId) => {
    ...
    try {
      await axios.post(
        `http://localhost:10000/api/community/projects/${projectId}/contribute`,
        { amount: 1 },
        {
          headers: {
